...
const ServiceCard: React.FC<{
  ...
  <div 
    ...
    /* --- AQUÍ LA SOMBRA VIOLETA --- */
    className={`group cursor-pointer relative h-full rounded-3xl overflow-hidden shadow-lg shadow-magenta/20 hover:shadow-2xl hover:shadow-magenta/40 transition-all duration-500 hover:-translate-y-2 ${featured ? 'md:scale-105 z-10' : ''}`}
  >
    <img 
      ...
      src={service.img}
    />
    {/* Overlay para legibilidad con tono Navy/Magenta */}
    <div className="absolute inset-0 bg-gradient-to-br from-slate-900/90 via-slate-900/70 to-magenta/60 opacity-90 transition-opacity"></div>
    ...
    <section id="servicios" className="py-24 bg-white dark:bg-slate-900 scroll-mt-20">
      <div className="max-w-[95%] 2xl:max-w-screen-2xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="text-center max-w-3xl mx-auto mb-20">
          <div className="w-20 h-1.5 bg-gradient-to-r from-blue-600 to-magenta mb-6 rounded-full mx-auto"></div>
          ...
          <p className="text-lg text-slate-600 dark:text-slate-400 font-medium">
            Explore nuestras áreas de especialidad diseñadas para transformar organizaciones.
          </p>
        </div>
      ...
      {/* Modal - Lo mantengo simplificado para el componente completo */}
      {selectedService && (
        <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 animate-fade-in">
          <div className="absolute inset-0 bg-slate-950/80 backdrop-blur-sm" onClick={() => setSelectedService(null)}></div>
          <div className="relative bg-white dark:bg-slate-900 w-full max-w-5xl max-h-[90vh] rounded-[2.5rem] overflow-hidden flex flex-col md:flex-row animate-scale-up">
            ...
            <div className="w-full md:w-3/5 p-8 md:p-12 overflow-y-auto">
               <button onClick={() => setSelectedService(null)} className="absolute top-6 right-6 text-slate-400 hover:text-magenta transition-colors"><span className="material-symbols-outlined">close</span></button>
               <h3 className="text-3xl font-bold text-slate-900 dark:text-white mb-6">{selectedService.title}</h3>
               <p className="text-slate-600 dark:text-slate-300 mb-8">{selectedService.fullDesc}</p>
