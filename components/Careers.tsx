...
const Careers: React.FC = () => {
  ...
  return (
    <section id="talento" className="py-12 bg-transparent overflow-hidden relative group transition-colors duration-300 scroll-mt-20">
      
      <div className="max-w-[95%] 2xl:max-w-screen-2xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
        {/* RECUADRO CON COLORES FIJOS DE MARCA */}
        <div className="bg-gradient-to-br from-[#0f172a] via-[#7b2cbf] to-magenta rounded-[2.5rem] p-8 md:p-12 flex flex-col md:flex-row items-center justify-between gap-8 relative overflow-hidden shadow-2xl transition-all hover:shadow-magenta/25 border border-white/10">
          ...
          <div className="relative z-10 shrink-0 w-full md:w-auto flex justify-center">
            <button 
              ...
              className="group/btn relative inline-flex items-center justify-center gap-3 px-8 py-5 bg-white text-[#7b2cbf] font-bold rounded-2xl overflow-hidden transition-all hover:scale-105 hover:shadow-[0_0_30px_rgba(255,255,255,0.4)] w-full md:w-auto border border-white/50 shadow-xl cursor-pointer"
            >
      ...
      {showForm && (
        <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 md:p-8 animate-fade-in bg-slate-950/80 backdrop-blur-sm overflow-y-auto">
          <div className="relative w-full max-w-4xl bg-white dark:bg-slate-900 rounded-[2.5rem] shadow-2xl overflow-hidden border border-white/10 my-auto animate-scale-up">
             ...
             <div className="bg-gradient-to-r from-[#0f172a] to-magenta p-8 md:p-10 relative overflow-hidden text-white text-center">
                ...
                {status === 'success' ? (
                   <div className="text-center py-12">
                    ...
                    <form onSubmit={handleSubmit} className="space-y-6" encType="multipart/form-data">
                        {/* El resto del formulario se mantiene igual a tu código original */}
                        <div className="grid md:grid-cols-2 gap-6">
                            <div>
                             ...
                             <div>
                                <label className="block text-sm font-bold mb-2 text-slate-700 dark:text-slate-300">Área de Interés *</label>
                        ...
                        <div className="grid md:grid-cols-2 gap-6">
                            <div>
                                <label className="block text-sm font-bold mb-2 text-slate-700 dark:text-slate-300">Email de Contacto *</label>
                        ...
                        <div>
                            <label className="block text-sm font-bold mb-2 text-slate-700 dark:text-slate-300">Adjuntar CV *</label>
                            <div className="p-6 border-2 border-dashed border-slate-200 dark:border-slate-700 rounded-2xl bg-slate-50 dark:bg-slate-800/50 hover:bg-slate-100 dark:hover:bg-slate-800 transition-colors text-center relative group">
                                <input type="file" name="cv" onChange={handleFileChange} accept=".pdf,.doc,.docx" required className="absolute inset-0 w-full h-full opacity-0 cursor-pointer z-10" disabled={status === 'submitting'} />
                                <div className="flex flex-col items-center gap-2">
                                    <span className="material-symbols-outlined text-4xl text-primary dark:text-accent">cloud_upload</span>
                                    <span className="font-bold text-slate-700 dark:text-white">{fileName || "Haga clic o arrastre su CV aquí"}</span>
                                </div>
                            </div>
                        </div>
                        <button type="submit" disabled={status === 'submitting'} className="w-full btn-gradient text-white py-5 rounded-xl font-bold text-lg shadow-xl shadow-primary/20 hover:opacity-95 transition-all transform active:scale-[0.98] flex items-center justify-center gap-3">
