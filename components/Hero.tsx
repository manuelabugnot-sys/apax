    ...
    <section id="inicio" className="relative min-h-screen flex items-center overflow-hidden scroll-mt-20 bg-white dark:bg-slate-950 transition-colors duration-300">
      ...
      <div className="max-w-[95%] 2xl:max-w-screen-2xl mx-auto px-4 sm:px-6 lg:px-8 relative z-20 w-full pt-28 md:pt-40 pb-24 md:pb-32">
        <div className="max-w-4xl">
          {/* Badge superior */}
          <div className="inline-flex items-center gap-2 px-4 py-1.5 rounded-full bg-slate-100 dark:bg-white/10 backdrop-blur-md text-[#0a008a] dark:text-accent font-bold text-sm mb-8 border border-slate-200">
          ...
          {/* TÍTULO: Tamaño respetado del código original (text-4xl lg:text-7xl) */}
          <h1 className="text-4xl lg:text-7xl font-extrabold tracking-tight mb-8 leading-[1.1] text-slate-900 dark:text-white drop-shadow-sm">
          ...
          <p className="text-lg lg:text-xl text-slate-600 dark:text-slate-300 font-medium mb-12 leading-relaxed max-w-3xl">
            Ofrecemos soluciones estratégicas de RRHH que conecten los objetivos organizacionales con la excelencia del equipo.
          </p>
          
          {/* BOTONES: Estilo de la imagen con la lógica de tu código */}
          <div className="flex flex-wrap gap-5">
            <button 
              onClick={() => scroll('contacto')}
              className="bg-gradient-to-r from-[#1a008a] to-[#7b2cbf] text-white px-10 py-5 rounded-full text-lg font-bold shadow-2xl hover:scale-105 active:scale-95 transition-all flex items-center justify-center gap-3"
            >
              Agendar Consultoría
            ...
            <button 
              ...
              className="bg-transparent border border-slate-200 dark:border-white/20 px-8 py-5 rounded-full text-lg font-bold hover:bg-slate-100 dark:hover:bg-white/5 transition-all text-slate-800 dark:text-white flex items-center justify-center gap-2 group"
            >
